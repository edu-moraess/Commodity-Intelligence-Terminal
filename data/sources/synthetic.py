...
def generate_price_series(ticker: str, days: int = 730, seed: int | None = None) -> pd.DataFrame:
    ...
    daily_range = close * (daily_vol * rng.uniform(0.5, 1.5, size=days))
    high = close + daily_range * rng.uniform(0.2, 0.6, size=days)
    low = close - daily_range * rng.uniform(0.2, 0.6, size=days)
    open_ = low + (high - low) * rng.uniform(0.3, 0.7, size=days)
    volume = rng.integers(5_000, 250_000, size=days)
    open_interest = rng.integers(20_000, 900_000, size=days)
...
def generate_macro_series(code: str, days: int = 1825) -> pd.Series:
    ...
    steps = rng.normal(0, base * 0.0015, size=days)
